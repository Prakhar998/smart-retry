...
export class StatsCollector {
  ...
  recordSuccess(endpoint: string, attemptNumber: number, recoveryTime?: number): void {
    ...
    if (recoveryTime !== undefined && attemptNumber > 1) {
      ...
      while (stat.recentRecoveryTimes.length > this.config.maxHistorySamples) stat.recentRecoveryTimes.shift();
      stat.avgRecoveryTime = this.calculateAverage(stat.recentRecoveryTimes);
    }
    if (stat.consecutiveFailures > 0) this.recordStreakOutcome(stat, stat.consecutiveFailures, true);
  ...
  recordExhausted(endpoint: string): void {
    const stat = this.getOrCreate(endpoint);
    if (stat.consecutiveFailures > 0) this.recordStreakOutcome(stat, stat.consecutiveFailures, false);
    this.scheduleSave(endpoint);
  }

  getStats(endpoint: string): EndpointStats | null { return this.stats.get(endpoint) || null; }
  getEndpoints(): string[] { return Array.from(this.stats.keys()); }
  clearStats(endpoint: string): void { this.stats.delete(endpoint); this.storageAdapter?.delete(endpoint); }
  clearAll(): void { const endpoints = this.getEndpoints(); this.stats.clear(); if (this.storageAdapter) endpoints.forEach(ep => this.storageAdapter!.delete(ep)); }
  ...
  async loadFromStorage(): Promise<void> {
    ...
    for (const key of keys) {
      ...
      if (stat) {
        const entries = Object.entries(stat.streakOutcomes || {}).map(([k, v]) => [parseInt(k, 10), v] as [number, { succeeded: number; failed: number }]);
  ...
  getSuccessProbability(endpoint: string, attemptNumber: number): number {
    ...
    if (!stat) return Math.pow(0.7, attemptNumber - 1);
    const bucket = Math.min(attemptNumber, 10);
    const outcomes = stat.streakOutcomes.get(bucket);
    if (outcomes && (outcomes.succeeded + outcomes.failed) >= 5) return outcomes.succeeded / (outcomes.succeeded + outcomes.failed);
  ...
  getRecoveryEstimate(endpoint: string): number {
    const stat = this.stats.get(endpoint);
    if (!stat || stat.recentRecoveryTimes.length === 0) return 1000;
  ...
  private getOrCreate(endpoint: string): EndpointStats {
    if (!this.stats.has(endpoint)) this.stats.set(endpoint, this.createEmptyStats(endpoint));
    return this.stats.get(endpoint)!;
  }

  private createEmptyStats(endpoint: string): EndpointStats {
    return {
      ...
      errorCategoryCounts: { [ErrorCategory.TRANSIENT]: 0, [ErrorCategory.OVERLOAD]: 0, [ErrorCategory.TIMEOUT]: 0, [ErrorCategory.PERMANENT]: 0, [ErrorCategory.UNKNOWN]: 0 },
    };
  }

  private updateRate(currentRate: number, attempts: number, success: boolean): number {
  ...
  private recordStreakOutcome(stat: EndpointStats, streak: number, succeeded: boolean): void {
    const bucket = Math.min(streak, 10);
    if (!stat.streakOutcomes.has(bucket)) stat.streakOutcomes.set(bucket, { succeeded: 0, failed: 0 });
    const outcome = stat.streakOutcomes.get(bucket)!;
    if (succeeded) outcome.succeeded++; else outcome.failed++;
  }

  private scheduleSave(endpoint: string): void {
    if (!this.storageAdapter) return;
    ...
    setTimeout(() => {
      if (this.pendingSaves.has(endpoint)) {
        ...
        const stat = this.stats.get(endpoint);
        if (stat) {
          const toStore = { ...stat, streakOutcomes: Object.fromEntries(stat.streakOutcomes) } as unknown as EndpointStats;
